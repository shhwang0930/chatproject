    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.SUBSCRIBE == accessor.getCommand()) { // 채팅룸 구독요청
            ...
            String name = Optional.ofNullable((Principal) message.getHeaders().get("simpUser"))
                    ...
                    .orElse("UnknownUser");
            chatService.sendChatMessage(MessageDTO.builder().type(MessageDTO.MessageType.ENTER).roomId(roomId).sender(name).build());
            ...
            String name = Optional.ofNullable((Principal) message.getHeaders().get("simpUser"))
                    .map(Principal::getName)
                    .orElse("UnknownUser");
            chatService.sendChatMessage(MessageDTO.builder().type(MessageDTO.MessageType.QUIT).roomId(roomId).sender(name).build());
